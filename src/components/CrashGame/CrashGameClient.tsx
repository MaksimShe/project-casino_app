...
interface CrashGameClientProps {
  initialGameState: CrashCurrentGame | null;
  initialHistory: CrashHistoryResponse | null;
}

export default function CrashGameClient({
  initialGameState,
  initialHistory,
}: CrashGameClientProps) {
  ...
  useEffect(() => {
    if (initialGameState) {
      ...
      store.setGameId(initialGameState.gameId);
      store.setGameState(initialGameState.state);
      store.setServerSeedHash(initialGameState.serverSeedHash);
      if (initialGameState.multiplier !== undefined) {
        store.setMultiplier(initialGameState.multiplier);
      }
      if (initialGameState.myBet) {
        store.setMyBet(initialGameState.myBet);
      }
    }
  }, [initialGameState]);
  ...
  const globalGames = useMemo(() => {
    if (initialHistory && 'games' in initialHistory) {
      return initialHistory.games;
    }
    return allGamesData && 'games' in allGamesData ? allGamesData.games : [];
  }, [initialHistory, allGamesData]);
